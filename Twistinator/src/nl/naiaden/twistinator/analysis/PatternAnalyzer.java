package nl.naiaden.twistinator.analysis;
 ...
 * rather than a {@link java.io.Reader}, that can flexibly separate text into
 * terms via a regular expression {@link Pattern} (with behaviour identical to
 * {@link String#split(String)}), and that combines the functionality of
 ...
 * <p>
 * This class can be considerably faster than the "normal" Lucene tokenizers. It
 * can also serve as a building block in a compound Lucene
...
public final class PatternAnalyzer extends Analyzer
{
			...
			do
			{
				...
				{ // found beginning; now find end of token
					...
					if (toLowerCase)
					{
						...
						// if (toLowerCase) {
						// // use next line once JDK 1.5 String.toLowerCase()
						...
						// // for (int j=start; j < i; j++) chars[j-start] =
	...
	private static final class PatternTokenizer extends TokenStream
	{
		...
		public final boolean incrementToken()
		{
			...
			clearAttributes();
			while (true)
			{ // loop takes care of leading and trailing boundary
				// cases
				int start = pos;
				int end;
	...
	/**
	 * <code>"\\W+"</code>; Divides text at non-letters (NOT
	 * Character.isLetter(c))
	 */
	public static final Pattern NON_WORD_PATTERN = Pattern.compile("\\W+");

	/**
	 * <code>"\\s+"</code>; Divides text at whitespaces
	...
	private static final CharArraySet EXTENDED_ENGLISH_STOP_WORDS = CharArraySet.unmodifiableSet(new CharArraySet(Version.LUCENE_34, Arrays.asList("a", "about", "above",
			...
			"during", "each", "eg", "either", "else", "elsewhere", "enough", "etc", "even", "ever", "every", "everyone", "everything", "everywhere", "except", "few", "first",
			"for", "former", "formerly", "from", "further", "had", "has", "have", "he", "hence", "her", "here", "hereafter", "hereby", "herein", "hereupon", "hers", "herself",
			"him", "himself", "his", "how", "however", "i", "ie", "if", "in", "inc", "indeed", "into", "is", "it", "its", "itself", "last", "latter", "latterly", "least", "less",
			"ltd", "many", "may", "me", "meanwhile", "might", "more", "moreover", "most", "mostly", "much", "must", "my", "myself", "namely", "neither", "never", "nevertheless",
			"next", "no", "nobody", "none", "noone", "nor", "not", "nothing", "now", "nowhere", "of", "off", "often", "on", "once one", "only", "onto", "or", "other", "others",
			"otherwise", "our", "ours", "ourselves", "out", "over", "own", "per", "perhaps", "rather", "s", "same", "seem", "seemed", "seeming", "seems", "several", "she",
			"should", "since", "so", "some", "somehow", "someone", "something", "sometime", "sometimes", "somewhere", "still", "such", "t", "than", "that", "the", "their", "them",
			"themselves", "then", "thence", "there", "thereafter", "thereby", "therefor", "therein", "thereupon", "these", "they", "this", "those", "though", "through",
			"throughout", "thru", "thus", "to", "together", "too", "toward", "towards", "under", "until", "up", "upon", "us", "very", "via", "was", "we", "well", "were", "what",
			"whatever", "whatsoever", "when", "whence", "whenever", "whensoever", "where", "whereafter", "whereas", "whereat", "whereby", "wherefrom", "wherein", "whereinto",
			"whereof", "whereon", "whereto", "whereunto", "whereupon", "wherever", "wherewith", "whether", "which", "whichever", "whichsoever", "while", "whilst", "whither",
			"who", "whoever", "whole", "whom", "whomever", "whomsoever", "whose", "whosoever", "why", "will", "with", "within", "without", "would", "xsubj", "xcal", "xauthor",
			"xother ", "xnote", "yet", "you", "your", "yours", "yourself", "yourselves"), true));
	/**
	...
	/**
	 * Reads until end-of-stream and returns all read chars, finally closes the
	 * stream.
	 * 
	 * @param input
	 *            the input stream
	 ...
	 * 
	 * @param matchVersion
	 *            If >= {@link Version#LUCENE_29},
	 *            StopFilter.enablePositionIncrement is set to true
	 * @param pattern
	 ...
	 * and is less efficient than <code>tokenStream(String, String)</code>.
	 * 
	 * @param fieldName
	...
	{
		// Ideally the Analyzer superclass should have a method with the same
		// signature,
		// with a default impl that simply delegates to the StringReader
		// flavour.
		if (text == null) {
